...
class TestVoteParser(unittest.TestCase):
	...
	def check(self, input, expected_valid, expected_values):
		p = VoteParser()
		(valid, values) = p.tryParseVote(input)
	...
	def checkCalc(self, input, expected):
		p = VoteParser()
		values = p.calculateResults(input)
		self.assertEqual(values, expected)
	
	def test_calculateResults(self):
		self.checkCalc({"Cal" : [5.0]}, [5.0]);             # One person
		self.checkCalc({"Cal" : [5.0, 6.5]}, [5.0, 6.5]);   # One persion, multiburp

		self.checkCalc({"Cal" : [5.0], "Beep" : [3.1]}, [4.05]);                 # Two people
		self.checkCalc({"Cal" : [5.0, 6.5], "Beep" : [3.1, 6.8]}, [4.05, 6.65]); # Two people multiburp
		self.checkCalc({"Cal" : [5.0, 6.5], "Beep" : [3.1]}, [4.05, 6.5]); # Two people, multiburp only by person 1
		self.checkCalc({"Cal" : [5.0], "Beep" : [3.1, 6.8]}, [4.05, 6.8]); # Two people, multiburp only by person 2
